import { buildParticipantColumns } from "../forms/participant-details.mjs";
...
export function normalizePricingPackages(packages = []) {
  return (Array.isArray(packages) ? packages : [])
    .map((pkg) => {
      ...
      return {
        id: String(pkg?.id || randomId()).trim(),
        label,
        amount,
...
export function normalizeParticipantTemplate(template = []) {
  return (Array.isArray(template) ? template : [])
    .map((field) => {
      ...
      return {
        id: String(field?.id || randomId()).trim(),
        label,
        type,
...
export function flattenParticipantDetailsForExport(participants = [], labelPrefix = "Niño") {
  return buildParticipantColumns(participants, labelPrefix);
}
