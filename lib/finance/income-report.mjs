...
function buildBankRow({ payment, submission, formTitle, bankAccount }) {
  ...
  return {
    ...
    bank: cleanText(data.bank_name) || cleanText(bankAccount?.bank_name),
    accountType: cleanText(bankAccount?.account_type),
    accountNumber: cleanText(bankAccount?.account_number),
    name: cleanText(data.sender_name) || findNameInReportSubmission(submission),
...
function buildManualRow({ submission, formTitle, bankAccount, mode }) {
  ...
  return {
    ...
    accountType: cleanText(bankAccount?.account_type),
    accountNumber: cleanText(bankAccount?.account_number),
    name: findNameInReportSubmission(submission),
    concept: cleanText(formTitle).toUpperCase(),
    amount,
...
export function buildFinanceIncomeReport({ formTitle = "Ingresos", bankAccount = null, submissions = [] } = {}) {
  const rows = [];
  ...
  for (const submission of submissions || []) {
    ...
    payments.filter(isActivePayment).forEach((payment) => {
      ...
      rows.push(buildBankRow({ payment, submission, formTitle, bankAccount }));
    });
  }
