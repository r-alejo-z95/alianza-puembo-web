...
function buildReconciliationStatus(payment) {
  return payment?.status === "verified" && payment?.bank_transaction_id
    ? "CONCILIADO"
    : "NO CONCILIADO";
}
...
function buildBankRow({ payment, submission, formTitle, bankAccount, bankAccountById, bankTransactionById }) {
  ...
  const registrantName = findNameInSubmission(submission);
  ...
  return {
    ...
    eventTotal: 0,
    reconciliationStatus: buildReconciliationStatus(payment),
...
export function buildFinanceIncomeReport({
  ...
  for (const submission of submissions || []) {
    ...
    if (coverageMode === "covered_by_used_payment" && activePayments.length === 0) {
      continue;
    }
