...
function normalizeTrackingPayment(payment) {
  if (!payment?.manual_disposition) return payment;

  return {
    ...payment,
    original_status: payment.original_status || payment.status || null,
    status: "rejected",
  };
}
...
export function getTrackingPaymentAmount(payment) {
  if (payment?.manual_disposition || payment?.status === "rejected") return 0;
  return toAmount(payment?.extracted_data?.amount ?? payment?.amount_claimed);
}

export function getActiveTrackingPayments(payments = []) {
  return (payments || []).filter(
    (payment) => !payment?.manual_disposition && payment?.status !== "rejected",
  );
}

export function getSubmissionTrackingPayments(submission) {
  const payments = Array.isArray(submission?.form_submission_payments)
    ? submission.form_submission_payments.map(normalizeTrackingPayment)
    : [];
