...
function expectedAmountForSubmission(totalAmount, submission) {
  const snapshotAmount = toAmount(submission?.expected_amount);
  if (snapshotAmount > 0) return snapshotAmount;
  return Math.max(toAmount(totalAmount), 0);
}
...
export function getSubmissionBalanceSummary({ submission = {}, totalAmount = 0 } = {}) {
  ...
  if (paymentGroup) {
    ...
    const expectedAmount = toAmount(
      paymentGroup.expected_amount || paymentGroup.calculated_expected_amount,
    );
  ...
  const normalizedTotal = expectedAmountForSubmission(totalAmount, submission);
