...
import { buildPricingSnapshot } from "./pricing-packages.mjs";
...
export function validateManualFinancialForm(form) {
  ...
  if (form.pricing_mode === "packages") {
    return {
      valid: false,
      error: "Las inscripciones manuales para formularios con paquetes se registran desde el formulario público por ahora.",
    };
  }
  return { valid: true };
}

export function buildManualPricingSnapshot(form, values = {}) {
  if (!form?.is_financial) {
    return { pricingSnapshot: null, expected_amount: null };
  }

  const selectedPackageId = values?.[form.pricing_field_id] || null;
