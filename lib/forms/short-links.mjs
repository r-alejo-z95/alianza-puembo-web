...
export const FORM_SHORT_LINK_ORIGIN = "https://alianzapuembo.org";

const RESERVED_FORM_SHORT_CODES = new Set([
  "admin",
  "api",
  "brand",
  "conocenos",
...
export function isReservedFormShortCode(code) {
  return RESERVED_FORM_SHORT_CODES.has(normalizeFormShortCode(code));
}
...
export function getFormShortUrl(shortCode, origin = FORM_SHORT_LINK_ORIGIN) {
  const cleanOrigin = String(origin || FORM_SHORT_LINK_ORIGIN).replace(/\/+$/g, "");
  return `${cleanOrigin}/${normalizeFormShortCode(shortCode)}`;
}

export function getFormShortDisplayUrl(shortCode, origin = FORM_SHORT_LINK_ORIGIN) {
  return getFormShortUrl(shortCode, origin).replace(/^https?:\/\//, "");
}
...
export async function findAvailableFormShortCode(supabase, titleOrCode, options = {}) {
  ...
  for (let attempt = 1; attempt <= 100; attempt += 1) {
    ...
    if (isReservedFormShortCode(candidate)) continue;
