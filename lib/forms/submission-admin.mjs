...
function getPricingFieldValue(form, values = {}) {
  ...
  let rawValue = Object.prototype.hasOwnProperty.call(values, fieldId)
    ? values[fieldId]
    : null;

  const field = (form?.form_fields || []).find((item) => item.id === fieldId);
  if (rawValue == null && field?.label && Object.prototype.hasOwnProperty.call(values, field.label)) {
    rawValue = values[field.label];
  }

  const normalizedValue = rawValue && typeof rawValue === "object"
    ? rawValue.value ?? rawValue.id ?? rawValue.label
    : rawValue;
  const packageMatch = normalizePricingPackages(form.pricing_packages).find(
    (pkg) =>
      String(pkg.id) === String(normalizedValue ?? "") ||
      pkg.label === String(normalizedValue ?? "") ||
      formatPackageOptionLabel(pkg) === String(normalizedValue ?? ""),
  );

  return packageMatch?.id || normalizedValue || null;
}
