...
export function useForms({ initialForms = [], isInternal = null } = {}) {
  ...
  const fetchForms = useCallback(async (options = {}) => {
  ...
  useEffect(() => {
    // Refetch in background to clear stale server cache while keeping the initial render.
    fetchForms({ silent: initialForms.length > 0 });
  }, [fetchForms, initialForms.length]);
  ...
  const archiveForm = useCallback(async (formId) => {
    ...
    try {
      ...
      await revalidateForms(); // Revalidate server cache
      await fetchForms({ silent: true });
      return true;
    } catch (err) {
      setForms(prevForms);
  ...
  const permanentlyDeleteForm = useCallback(async (formId) => {
    ...
    try {
      ...
      const { data: formToDelete } = await supabase
        .from("forms")
        .select("image_url, form_fields!form_id(attachment_url)")
        .eq("id", formId)
        .single();
      ...
      // No need to revalidate cache for permanent delete of archived items
      await fetchForms({ silent: true });
      return true;
    } catch (err) {
      setArchivedForms(prevArchived);
