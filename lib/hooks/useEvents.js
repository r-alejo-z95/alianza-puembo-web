...
export function useEvents({ initialEvents = [] } = {}) {
  ...
  const saveEvent = useCallback(async (eventData, posterFile, selectedEvent = null) => {
    ...
    // Handle poster removal or replacement
    ...
    // Create form if necessary (registration_type === 'auto')
    if (eventData.create_form) {
      setIsCreatingForm(true);
      try {
        const formCreationResult = await createFormAndSheet(eventData.title);
        ...
        if (success) {
          ...
          toast.success('Formulario de registro creado con éxito.');
        } else {
    ...
    await fetchEvents();
    return { success: true, formId: dataToSave.form_id };
  ...
  const calendarEvents = events.flatMap(event => {
    ...
    // --- RECURRING EVENTS LOGIC (Expansion) ---
    const instances = [];
