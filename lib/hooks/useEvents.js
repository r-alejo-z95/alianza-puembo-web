...
import { revalidateEvents, revalidateForms } from '@/lib/actions/cache';
...
export function useEvents({ initialEvents = [] } = {}) {
  ...
  const saveEvent = useCallback(async (eventData, posterFile, selectedEvent = null) => {
    ...
    const baseSlug = selectedEvent && selectedEvent.title === eventData.title
      ? selectedEvent.slug
      : slugify(eventData.title);

    let slug = baseSlug;

    // Si es un evento nuevo o el título cambió, verificar colisión de slug
    if (!selectedEvent || selectedEvent.title !== eventData.title) {
      ...
      if (existing && existing.id !== selectedEvent?.id) {
        // Opción A: Añadir sufijo aleatorio (más amigable que un error)
    ...
    if (eventData.create_form) {
      ...
      const formSlug = slugify(eventData.title);
      const shortCode = await findAvailableFormShortCode(supabase, eventData.title);
      ...
      if (formError || !newForm) {
        ...
        toast.error('No se pudo crear el formulario del evento.');
        return { success: false };
      }

      dataToSave.registration_link = `/formularios/${newForm.slug}`;
      dataToSave.form_id = newForm.id;
      createdFormSlug = newForm.slug;
    ...
    await revalidateEvents(); // Invalidate server cache
    await fetchEvents(); // Update local state
  ...
  const archiveEvents = useCallback(async (eventIds) => {
    const previousEvents = [...events];
    setEvents(current => current.filter(e => !eventIds.includes(e.id)));
    ...
    try {
      const { error } = await supabase
        ...
        .in('id', eventIds);
      if (error) throw error;
      toast.success(`${eventIds.length} eventos movidos a la papelera`);
      await revalidateEvents(); // Invalidate server cache
      return true;
    } catch (err) {
      setEvents(previousEvents);
      toast.error('Error al archivar eventos');
      return false;
    }
  }, [supabase, events]);
  ...
  const restoreEvents = useCallback(async (eventIds) => {
    const previousArchived = [...archivedEvents];
    setArchivedEvents(current => current.filter(e => !eventIds.includes(e.id)));
    ...
    try {
      const { error } = await supabase
        ...
        .in('id', eventIds);
      if (error) throw error;
      toast.success(`${eventIds.length} eventos restaurados`);
      await revalidateEvents(); // Invalidate server cache
      fetchEvents();
      return true;
    } catch (err) {
      setArchivedEvents(previousArchived);
      toast.error('Error al restaurar eventos');
      return false;
    }
  }, [supabase, archivedEvents, fetchEvents]);
  ...
  const permanentlyDeleteEvent = useCallback(async (eventId) => {
    ...
    try {
      ...
      await revalidateEvents(); // Invalidate server cache if somehow it affects public view (unlikely for perma-delete from archive, but good practice)
      return true;
    } catch (err) {
      setArchivedEvents(previousArchived);
  ...
  const permanentlyDeleteEvents = useCallback(async (eventIds) => {
    const previousArchived = [...archivedEvents];
    setArchivedEvents(current => current.filter(e => !eventIds.includes(e.id)));

    try {
      // 1. Obtener posters para borrar de storage
      const { data: eventsToDelete } = await supabase.from('events').select('poster_url').in('id', eventIds);
      const postersToDelete = eventsToDelete
        ?.map(e => e.poster_url?.split('/').pop())
        .filter(Boolean)
        .map(name => decodeURIComponent(name));

      if (postersToDelete?.length > 0) {
        await supabase.storage.from('event-posters').remove(postersToDelete);
      }
      ...
      toast.success(`${eventIds.length} eventos eliminados definitivamente`);
      return true;
    } catch (err) {
      setArchivedEvents(previousArchived);
  ...
  const emptyRecycleBin = useCallback(async () => {
    ...
    return permanentlyDeleteEvents(eventIds);
  }, [archivedEvents, permanentlyDeleteEvents]);
  ...
  return {
    ...
    saveEvent, archiveEvent, archiveEvents, restoreEvent, restoreEvents, 
    permanentlyDeleteEvent, permanentlyDeleteEvents, emptyRecycleBin,
