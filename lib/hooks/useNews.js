...
export function useNews({ initialNews = [] } = {}) {
    ...
    async (newsData, imageFile, selectedNews = null) => {
      ...
      if (newsData.remove_image || imageFile) {
        if (selectedNews && selectedNews.image_url) {
          ...
          await supabase.storage.from("news-images").remove([decodeURIComponent(fileName)]);
          image_url = null; image_w = null; image_h = null;
        }
      }
      ...
      if (imageFile) {
        ...
        const { data: uploadData, error: uploadError } = await supabase.storage
          .from("news-images").upload(fileName, imageFile.file);
        ...
        if (uploadError) {
          ...
          image_url = urlData.publicUrl; image_w = imageFile.width; image_h = imageFile.height;
        }
      }
      ...
      if (selectedNews?.id) {
        const { error } = await supabase.from("news").update(dataToSave).eq("id", selectedNews.id);
        if (error) { toast.error("Error al actualizar"); return { success: false }; }
      } else {
        const { error } = await supabase.from("news").insert([{ ...dataToSave, user_id: user?.id }]);
  ...
  const archiveNews = useCallback(async (newsId) => {
    const prev = [...news];
  ...
  const archiveManyNews = useCallback(async (newsIds) => {
    const prev = [...news];
  ...
  const restoreManyNews = useCallback(async (newsIds) => {
    const prevArchived = [...archivedNews];
    setArchivedNews(current => current.filter(n => !newsIds.includes(n.id)));
  ...
  const permanentlyDeleteNews = useCallback(async (newsId) => {
    const prevArchived = [...archivedNews];
    setArchivedNews(current => current.filter(n => n.id !== newsId));

    try {
      const { data: item } = await supabase.from("news").select("image_url").eq("id", newsId).single();
      if (item?.image_url) {
  ...
  const permanentlyDeleteManyNews = useCallback(async (newsIds) => {
    const prevArchived = [...archivedNews];
    setArchivedNews(current => current.filter(n => !newsIds.includes(n.id)));

    try {
      const { data: items } = await supabase.from("news").select("image_url").in("id", newsIds);
      ...
      const { error } = await supabase.from("news").delete().in("id", newsIds);
      if (error) throw error;
  ...
  return {
    news, archivedNews, loading, loadingArchived,
    saveNews, archiveNews, archiveManyNews, restoreNews, restoreManyNews,
    permanentlyDeleteNews, permanentlyDeleteManyNews, emptyRecycleBin,
    fetchArchivedNews, refetchNews: fetchNews,
  };
}