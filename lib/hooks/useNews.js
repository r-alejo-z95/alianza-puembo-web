...
export function useNews({ initialNews = [] } = {}) {
  ...
  const saveNews = useCallback(async (newsData, imageFile, selectedNews = null) => {
    ...
    if (imageFile) {
      if (selectedNews && selectedNews.image_url) {
        ...
        if (deleteOldStorageError) {
          console.error('Error deleting old image from storage:', deleteOldStorageError);
    ...
    // Prepare news data
    // Combine date and time to ISO string if provided, otherwise default to now or null logic?
    // Based on user request, they are optional.
    // If user provides date but not time, we can default to 00:00
    // If user provides nothing, we might default to now() or keep it null if DB allows.
    // However, for sorting purposes, it is better to have a date.
    // Let's implement: if date provided, use it. Time optional (default 00:00).
    // If date not provided, use current date/time.

    let isoDate = null;
    if (newsData.date) {
        const timePart = newsData.time || '00:00';
        isoDate = new Date(`${newsData.date}T${timePart}:00`).toISOString();
    }
  ...
  const deleteNews = useCallback(async (newsId) => {
    ...
    const { error: deleteDbError } = await supabase.from('news').delete().eq('id', newsId);
    if (deleteDbError) {
