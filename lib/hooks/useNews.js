...
export function useNews({ initialNews = [] } = {}) {
  ...
  const saveNews = useCallback(async (newsData, imageFile, selectedNews = null) => {
    ...
    // Prepare news data
    let isoDate = null;
    const has_date = Boolean(newsData.date);
    const has_time = Boolean(newsData.time);

    if (has_date || has_time) {
        const datePart = newsData.date || '0001-01-01';
        const timePart = newsData.time || '00:00';
        isoDate = new Date(`${datePart}T${timePart}:00`).toISOString();
    }
