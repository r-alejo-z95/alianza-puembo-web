...
export default function FormManager() {
  const {
    ...
    fetchArchivedForms, refetchForms
  } = useForms();
  ...
  useEffect(() => { localStorage.setItem("admin_forms_groupByMonth", JSON.stringify(groupByMonth)); }, [groupByMonth]);
  ...
  return (
    <div className="space-y-6">
      <Card className="border-none shadow-2xl bg-white rounded-[2.5rem] overflow-hidden">
        <CardHeader className="p-6 md:p-12 border-b border-gray-50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gray-50/30">
          <div className="space-y-1">
            ...
            <CardTitle className="text-3xl font-serif font-bold text-gray-900">Gestión de Formularios</CardTitle>
          </div>
          <div className="flex gap-3">
            ...
            <Button variant="green" className="hidden lg:flex rounded-full px-8 py-6 font-bold shadow-lg shadow-[var(--puembo-green)]/20 transition-all hover:-translate-y-0.5" onClick={() => router.push("/admin/formularios/builder")}>
          ...
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-3 w-full lg:w-auto">
              <div className="flex items-center bg-gray-50 p-1 rounded-full border border-gray-100 w-full lg:w-auto">
                ...
                <Button variant={sortConfig.key === "created_at" ? "green" : "ghost"} onClick={() => handleSort("created_at")} className={cn("flex-1 lg:flex-none rounded-full h-10 px-6 font-bold text-[9px] uppercase tracking-[0.2em] gap-2 transition-all", sortConfig.key === "created_at" ? "shadow-md" : "text-gray-400 hover:bg-gray-100")}>
        ...
        <CardContent className="p-0">
          {loading ? ( <ManagerSkeleton rows={itemsPerPage} columns={6} /> ) : processedForms.length === 0 ? (
            <div className="py-32 text-center space-y-4">
              <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto"><FileSpreadsheet className="w-8 h-8 text-gray-200" /></div>
              <p className="text-gray-400 font-light italic text-lg font-serif px-8">{searchTerm ? "No se encontraron formularios." : "No hay formularios creados."}</p>
            </div>
          ) : (
            <div id="forms-table">
                ...
                <Table className="w-full">
                  <TableHeader className="bg-gray-50/50">
                    <TableRow className="hover:bg-transparent border-b border-gray-100">
                      ...
                      <TableHead className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 text-center">Sheets</TableHead>
                      <TableHead className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 text-center">Drive</TableHead>
                      <TableHead className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 text-center">Autor</TableHead>
                      <TableHead className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 text-center">Acciones</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {Object.entries(groupedCurrentItems).map(([group, groupForms]) => (
                      <React.Fragment key={group}>
                        ...
                        {groupForms.map((form) => ( <FormRow key={form.id} form={form} isSelected={selectedIds.includes(form.id)} onSelect={() => toggleSelect(form.id)} onEdit={handleEdit} onDelete={handleDelete} compact={false} /> ))}
                      </React.Fragment>
                    ))}
                  </TableBody>
                </Table>
              ...
              <div className="lg:hidden p-6 space-y-12">
                {Object.entries(groupedCurrentItems).map(([group, groupForms]) => (
                   <div key={group} className="space-y-6">
                      ...
                      {groupForms.map((form) => ( <FormRow key={form.id} form={form} isSelected={selectedIds.includes(form.id)} onSelect={() => toggleSelect(form.id)} onEdit={handleEdit} onDelete={handleDelete} compact={true} /> ))}
                   </div>
                 ))}
              </div>
      ...
      <RecycleBin open={isRecycleBinOpen} onOpenChange={setIsRecycleBinOpen} type="forms" items={archivedForms} onRestore={restoreForm} onDelete={permanentlyDeleteForm} onBulkRestore={restoreManyForms} onBulkDelete={permanentlyDeleteManyForms} onEmptyTrash={emptyRecycleBin} loading={loadingArchived} />
      <AdminFAB onClick={() => router.push("/admin/formularios/builder")} label="Crear Formulario" />
    </div>
  );
}