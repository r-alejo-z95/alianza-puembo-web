...
export default function FormManager() {
  ...
  const handleSave = async (formData, imageFile) => {
    ...
    if (imageFile) {
      ...
      if (selectedForm && selectedForm.image_url) {
        ...
        if (deleteOldStorageError) {
          console.error('Error deleting old form image from storage:', deleteOldStorageError);
          toast.error('Error al eliminar la imagen antigua del almacenamiento.');
        }
      }
      ...
      if (uploadError) {
        ...
        const { data: urlData } = supabase.storage.from('form-images').getPublicUrl(uploadData.path);
    ...
    if (selectedForm) {
      ...
      const { data: newForm, error: formError } = await supabase
        .from('forms')
        .insert([{ title, description, image_url: imageUrl, user_id: user?.id, slug }])
        .select()
        .single();
  ...
  const handleDelete = async (formId) => {
    ...
    if (formToDelete.image_url) {
      ...
      if (deleteStorageError) {
        console.error('Error deleting form image from storage:', deleteStorageError);
  ...
  const totalPages = useMemo(() => Math.ceil(forms.length / itemsPerPage), [forms.length, itemsPerPage]);
    ...
    <Card>
                ...
                <TableBody>
                  {currentForms.map((form) => (
                    <FormRow
                      key={form.id}
                      form={form}
                      onEdit={() => { setSelectedForm(form); setIsFormOpen(true); }}
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-4xl overflow-y-auto max-h-[90vh]">
          <DialogHeader>
            <DialogTitle>{selectedForm ? 'Editar Formulario' : 'Crear Nuevo Formulario'}</DialogTitle>
          </DialogHeader>
          <FormBuilder
            form={selectedForm}
            onSave={handleSave}
