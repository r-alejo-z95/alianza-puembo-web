...
export default function FormManager() {
  ...
  useEffect(() => {
    localStorage.setItem("admin_forms_groupByMonth", JSON.stringify(groupByMonth));
  }, [groupByMonth]);
  ...
  const handleDelete = async (formId) => {
    ...
    if (success) {
      setSelectedIds(prev => prev.filter(id => id !== formId));
    }
  };
  ...
  // 1. Process search and sort on all forms
  ...
  const currentItems = useMemo(() => {
    ...
    return processedForms.slice(startIndex, endIndex);
  }, [processedForms, currentPage, itemsPerPage]);

  // 3. Group ONLY the current page items
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === currentItems.length && currentItems.length > 0) {
      ...
      setSelectedIds(currentItems.map(f => f.id));
    }
  };
        ...
        <div className="px-8 py-6 border-b border-gray-50 bg-white/50 backdrop-blur-sm sticky top-0 z-20">
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="relative w-full lg:max-w-md group">
              ...
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm("")} className="absolute right-5 top-1/2 -translate-y-1/2 p-1.5 hover:bg-gray-100 rounded-full transition-colors">
            ...
            <div className="flex items-center gap-3 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
              <div className="flex items-center bg-gray-50 p-1.5 rounded-full border border-gray-100">
                <Button 
                  ...
                  className={cn("rounded-full h-11 px-6 font-bold text-[10px] uppercase tracking-[0.2em] gap-2 transition-all", sortConfig.key === "title" ? "shadow-lg" : "text-gray-400 hover:bg-gray-100")}
                >
                  {sortConfig.key === "title" && (sortConfig.direction === "asc" ? <SortAsc className="w-3.5 h-3.5" /> : <SortDesc className="w-3.5 h-3.5" />)}
                  Nombre
                </Button>
                <Button 
                  variant={sortConfig.key === "created_at" ? "green" : "ghost"}
                  onClick={() => handleSort("created_at")}
                  className={cn("rounded-full h-11 px-6 font-bold text-[10px] uppercase tracking-[0.2em] gap-2 transition-all", sortConfig.key === "created_at" ? "shadow-lg" : "text-gray-400 hover:bg-gray-100")}
                >
                  {sortConfig.key === "created_at" && (sortConfig.direction === "asc" ? <SortAsc className="w-3.5 h-3.5" /> : <SortDesc className="w-3.5 h-3.5" />)}
                  Fecha
                </Button>
              </div>
              ...
              <Button 
                ...
                className={cn("rounded-full h-14 px-8 font-bold text-[10px] uppercase tracking-[0.2em] gap-3 transition-all", !groupByMonth && "border-gray-100 text-gray-500 hover:bg-gray-50")}
          ...
          <AnimatePresence>
            {selectedIds.length > 0 && (
              <motion.div 
                ...
                className="mt-6 p-4 bg-gray-900 rounded-full flex items-center justify-between shadow-2xl ring-4 ring-black/5 z-30"
              >
                ...
                <div className="flex gap-3">
                  ...
                  <Button className="bg-red-500 hover:bg-red-600 text-white rounded-full font-bold text-[10px] uppercase tracking-widest px-10 h-12 gap-3 shadow-xl transition-all" onClick={handleBulkArchive}>
          ...
          {loading ? (
            ...
            <div className="py-32 text-center space-y-4">
              ...
              <p className="text-gray-400 font-light italic text-lg font-serif px-8">
                {searchTerm ? "No se encontraron formularios." : "No hay formularios creados todavía."}
              </p>
            </div>
          ) : (
            <div id="forms-table">
