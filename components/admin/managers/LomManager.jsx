...
import {
  Loader2, BookOpen, ListFilter, PenTool, Trash2, Search, SortAsc, SortDesc, LayoutGrid, Rows, X, CheckCircle2, Plus,
} from "lucide-react";
import { useLom } from "@/lib/hooks/useLom";
import RecycleBin from "./RecycleBin";
import { slugify } from "@/lib/utils";

const RichTextEditor = dynamic(() => import("@/components/admin/forms/RichTextEditor"), { ssr: false });
...
export default function LomManager() {
  const {
    items: posts, archivedItems: archivedPosts, loading, loadingArchived,
    ...
    fetchArchivedItems, refetchItems,
  } = useLom({ type: "posts" });
  ...
  useEffect(() => { localStorage.setItem("admin_lom_groupByMonth", JSON.stringify(groupByMonth)); }, [groupByMonth]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (!selectedPost || selectedPost.title !== data.title) {
        const { data: existing } = await supabase.from("lom_posts").select("id").eq("slug", baseSlug).maybeSingle();
        if (existing && existing.id !== selectedPost?.id) { slug = `${baseSlug}-${Math.random().toString(36).substring(2, 6)}`; }
      }

      const dataToSave = { ...data, publication_date: data.publication_date, user_id: user?.id, slug: slug, is_archived: false };
      ...
      setIsFormOpen(false); setSelectedPost(null);
      form.reset({ title: "", content: "", publication_date: "" });
      setEditorKey((prev) => prev + 1); refetchItems();
    } catch (error) { toast.error("Error al guardar."); } finally { setSubmitting(false); }
  };

  const handleEdit = (post) => {
    setSelectedPost(post);
    form.reset({ title: post.title, content: post.content, publication_date: post.publication_date });
    setEditorKey((prev) => prev + 1); setIsFormOpen(true);
  };
    ...
    <div className="space-y-6">
      <Card className="border-none shadow-2xl bg-white rounded-[2.5rem] overflow-hidden">
        <CardHeader className="p-6 md:p-12 border-b border-gray-50 bg-gray-50/30 flex flex-col md:flex-row gap-3 justify-between">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-gray-400">
              <BookOpen className="w-3 h-3" /> <span>Archivo Editorial</span>
            </div>
            <CardTitle className="text-3xl font-serif font-bold text-gray-900">Gestión de LOM</CardTitle>
          </div>
          <div className="flex gap-3">
            ...
            <Button variant="green" className="hidden lg:flex rounded-full px-8 py-6 font-bold shadow-lg shadow-[var(--puembo-green)]/20 transition-all hover:-translate-y-0.5" onClick={() => { setSelectedPost(null); form.reset({ title: "", content: "", publication_date: "" }); setEditorKey((prev) => prev + 1); setIsFormOpen(true); }}>
        ...
        <div className="px-6 py-4 border-b border-gray-50 bg-white/50 backdrop-blur-sm sticky top-0 z-20">
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="relative w-full lg:max-w-md group">
              ...
              <Input placeholder="Buscar devocional..." className="pl-14 h-14 rounded-full bg-gray-50 border-gray-100 focus:bg-white transition-all text-sm font-medium focus:ring-4 focus:ring-[var(--puembo-green)]/10" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
            ...
            <div className="flex flex-col lg:flex-row items-center gap-3 w-full lg:w-auto">
              <div className="flex items-center bg-gray-50 p-1 rounded-full border border-gray-100 shrink-0 w-full lg:w-auto">
                ...
                <Button variant={sortConfig.key === "publication_date" ? "green" : "ghost"} onClick={() => handleSort("publication_date")} className={cn("flex-1 lg:flex-none rounded-full h-10 px-6 font-bold text-[9px] uppercase tracking-[0.2em] gap-2 transition-all", sortConfig.key === "publication_date" ? "shadow-md" : "text-gray-400 hover:bg-gray-100")}>
        ...
        <CardContent className="p-0">
          {loading ? ( <ManagerSkeleton rows={10} columns={4} /> ) : processedPosts.length === 0 ? (
            <div className="py-32 text-center space-y-4">
              ...
              <p className="text-gray-400 font-light italic text-lg font-serif px-8">{searchTerm ? "No se encontraron devocionales." : "No hay devocionales publicados."}</p>
            </div>
          ) : (
            <div id="lom-table">
                  ...
                  <TableBody>
                    {Object.entries(groupedCurrentItems).map(([group, groupPosts]) => (
                      <React.Fragment key={group}>
                        ...
                        {groupPosts.map((post) => ( <LomRow key={post.id} post={post} isSelected={selectedIds.includes(post.id)} onSelect={() => toggleSelect(post.id)} onEdit={handleEdit} onDelete={handleDelete} compact={false} /> ))}
                      </React.Fragment>
                    ))}
                  </TableBody>
                </Table>
      ...
      <AdminEditorPanel open={isFormOpen} onOpenChange={setIsFormOpen} title={<>{selectedPost?.id ? "Refinar" : "Crear"} <br /><span className="text-[var(--puembo-green)] italic">Devocional</span></>}>
        <div className="md:p-12 bg-white pb-12">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <FormField control={form.control} name="title" render={({ field }) => (
                    <FormItem><FormLabel className="text-xs font-black uppercase tracking-widest text-gray-400">Título de la Lectura</FormLabel><FormControl><Input placeholder="Ej: Caminando sobre las aguas" className="h-12 rounded-xl bg-gray-50 border-gray-100 focus:bg-white transition-all" {...field} /></FormControl><FormMessage /></FormItem>
                  )}
                />
                <FormField control={form.control} name="publication_date" render={({ field }) => (
                    <FormItem><FormLabel className="text-xs font-black uppercase tracking-widest text-gray-400">Fecha de Publicación</FormLabel><FormControl><Input type="date" className="h-12 rounded-xl bg-gray-50 border-gray-100 focus:bg-white transition-all" {...field} /></FormControl><FormMessage /></FormItem>
                  )}
                />
              </div>
              <FormField control={form.control} name="content" render={({ field }) => (
                  <FormItem><FormLabel className="text-xs font-black uppercase tracking-widest text-gray-400">Cuerpo del Devocional</FormLabel><FormControl><div className="rounded-2xl border border-gray-100 overflow-hidden shadow-inner"><RichTextEditor key={editorKey} content={field.value} onChange={field.onChange} /></div></FormControl><FormMessage /></FormItem>
                )}
              />
              <div className="flex justify-end gap-3 md:gap-4 pt-4">
                <Button type="button" variant="ghost" className="rounded-full px-4 md:px-8 text-xs md:text-sm font-bold" onClick={() => { setIsFormOpen(false); setSelectedPost(null); }}>Descartar</Button>
                <Button type="submit" disabled={submitting} variant="green" className="rounded-full px-6 md:px-10 py-5 md:py-7 text-xs md:text-sm font-bold shadow-lg shadow-[var(--puembo-green)]/20 transition-all hover:-translate-y-0.5">{submitting ? ( <Loader2 className="h-4 w-4 animate-spin mr-2" /> ) : ( <PenTool className="w-4 h-4 mr-2" /> )}{selectedPost ? "Guardar Cambios" : "Publicar Lectura"}</Button>
              </div>
            </form>
          </Form>
        </div>
      ...
      <AdminFAB onClick={() => { setSelectedPost(null); form.reset({ title: "", content: "", publication_date: "" }); setEditorKey((prev) => prev + 1); setIsFormOpen(true); }} label="Nuevo LOM" />
    </div>
  );
}