...
const formatNewsDate = (dateStr) => {
    ...
    const options = { 
        ...
        hour: '2-digit', 
        minute: '2-digit',
        timeZone: 'America/Guayaquil' // Assuming same timezone as events
    };
...
export function NewsRow({ newsItem, onEdit, onDelete, compact }) {
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" asChild>
                            <a href={newsItem.image_url} target="_blank" rel="noopener noreferrer" aria-label="Ver imagen">
    ...
    const actions = (
        <div className="flex items-center gap-1">
            <Button variant="outline" size="icon" aria-label="Editar noticia" onClick={onEdit}>
                <Edit className="w-4 h-4" />
            </Button>
            <AlertDialog>
                <AlertDialogTrigger asChild>
                    <Button variant="destructive" size="icon" aria-label="Eliminar noticia">
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Esta acción no se puede deshacer. Esto eliminará permanentemente la noticia de nuestros servidores.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
    ...
    const formattedDate = formatNewsDate(newsItem.date);
    ...
    if (compact) {
        return (
            <div className='border rounded-lg p-4 shadow-sm space-y-2'>
                ...
                {newsItem.description && <div><span className="font-semibold">Descripción:</span> {newsItem.description}</div>}
                <div><span className="font-semibold">Fecha:</span> {formattedDate}</div>
                {newsItem.image_url && <div><span className="font-semibold">Imagen:</span> {posterActions}</div>}
