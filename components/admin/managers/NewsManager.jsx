...
export default function NewsManager() {
    ...
    const totalPages = useMemo(() => Math.ceil(news.length / itemsPerPage), [news.length, itemsPerPage]);
    ...
    return (
        <Card className="mb-16">
            <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button onClick={() => { setSelectedNews(null); setIsFormOpen(true); }}>Añadir Noticia</Button>
            </CardHeader>
                                ...
                                <TableBody>
                                    {currentNews.map((item) => (
                                        <NewsRow
                                            key={item.id}
                                            newsItem={item}
                                            onEdit={() => { setSelectedNews(item); setIsFormOpen(true); }}
            ...
            <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{selectedNews?.id ? 'Editar Noticia' : 'Crear Nueva Noticia'}</DialogTitle>
                    </DialogHeader>
                    <NewsForm
