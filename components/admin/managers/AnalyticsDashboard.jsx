...
// Gráfico de área para tendencia (importación corregida)
...
export default function AnalyticsDashboard({ form, submissions: allSubmissions }) {
  ...
  const filteredSubmissions = useMemo(() => {
    ...
    switch (dateFilter) {
      case "today": {
        ...
        return allSubmissions.filter(s => formatInEcuador(s.created_at, "yyyy-MM-dd") === todayStr);
      }
      case "7days": {
        const sevenDaysAgo = subDays(nowUtc, 7);
        return allSubmissions.filter(s => new Date(s.created_at) >= sevenDaysAgo);
      }
      case "30days": {
        const thirtyDaysAgo = subDays(nowUtc, 30);
        return allSubmissions.filter(s => new Date(s.created_at) >= thirtyDaysAgo);
      }
      default:
  ...
  const last24Hours = useMemo(() => {
    ...
    return allSubmissions.filter(s => new Date(s.created_at) > twentyFourHoursAgo).length;
  }, [allSubmissions]);
  ...
  const activityData = useMemo(() => {
    ...
    const days = Array.from({ length: rangeSize }, (_, i) => format(subDays(nowEcuador, i), "yyyy-MM-dd")).reverse();
    return days.map(day => ({
      date: formatInEcuador(new Date(day + "T12:00:00"), "d MMM"),
      cantidad: allSubmissions.filter(s => formatInEcuador(s.created_at, "yyyy-MM-dd") === day).length
    }));
  }, [allSubmissions, dateFilter]);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-gray-100 pb-10">
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <h1 className="text-4xl md:text-6xl font-serif font-bold text-gray-900 tracking-tight leading-none">
              Analíticas <span className="text-[var(--puembo-green)] italic">de Datos</span>
            </h1>
            <div className="flex flex-wrap items-center gap-3 text-gray-400 font-light text-lg">
              <span>Resultados de:</span>
              <span className="font-medium text-gray-600 border-b-2 border-[var(--puembo-green)]/20">{form.title}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 bg-white p-2 rounded-[2.5rem] border border-gray-100 shadow-xl shadow-gray-200/20">
          <div className="hidden sm:flex items-center gap-3 px-5 py-2 border-r border-gray-100">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Periodo</span>
          </div>
          <Select value={dateFilter} onValueChange={setDateFilter}>
            ...
            <SelectContent className="rounded-2xl border-none shadow-2xl bg-black text-white">
              <SelectItem value="all" className="rounded-xl focus:bg-[var(--puembo-green)] focus:text-white">Todo el histórico</SelectItem>
              <SelectItem value="today" className="rounded-xl focus:bg-[var(--puembo-green)] focus:text-white">Hoy</SelectItem>
              <SelectItem value="7days" className="rounded-xl focus:bg-[var(--puembo-green)] focus:text-white">7 días</SelectItem>
              <SelectItem value="30days" className="rounded-xl focus:bg-[var(--puembo-green)] focus:text-white">30 días</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex justify-center mb-16 md:mb-20">
          <TabsList className="bg-gray-100/50 p-1.5 rounded-full border border-gray-200/50 backdrop-blur-sm h-auto flex-nowrap justify-center max-w-full">
            <TabsTrigger value="summary" className="rounded-full px-4 md:px-8 py-3 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-black data-[state=active]:text-white transition-all duration-500">
              <PieIcon className="w-3.5 h-3.5 md:mr-2" /> <span className="hidden md:inline">Resumen Estadístico</span><span className="md:hidden">Resumen</span>
            </TabsTrigger>
            <TabsTrigger value="individual" className="rounded-full px-4 md:px-8 py-3 text-[10px] font-black uppercase tracking-widest data-[state=active]:bg-black data-[state=active]:text-white transition-all duration-500">
              <User className="w-3.5 h-3.5 md:mr-2" /> <span className="hidden md:inline">Respuestas Individuales</span><span className="md:hidden">Individual</span>
            </TabsTrigger>
          </TabsList>
        </div>

        {/* --- CONTENIDO: RESUMEN --- */}
        <TabsContent value="summary" className="space-y-12 focus-visible:outline-none">
          {/* Fila de KPIs rápidos */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
            {[
              { label: "Respuestas", value: totalSubmissions, icon: Inbox, bg: "bg-green-50", color: "var(--puembo-green)" },
              { label: "Actividad 24h", value: last24Hours, icon: Activity, bg: "bg-blue-50", color: "#3b82f6" },
              { label: "Estado", value: form.enabled ? "Activo" : "Cerrado", icon: TrendingUp, bg: "bg-amber-50", color: "#f59e0b" }
            ].map((kpi, i) => (
              <Card key={i} className="border-none shadow-2xl bg-white rounded-[2.5rem] overflow-hidden group">
                <CardContent className="p-8 flex items-center gap-6">
                  <div className={cn("w-16 h-16 rounded-2xl flex items-center justify-center transition-transform group-hover:rotate-6", kpi.bg)}>
                  ...
                  <div>
                    ...
                    <h3 className="text-3xl font-serif font-bold text-gray-900">{kpi.value}</h3>
                  </div>
                </CardContent>
              </Card>
            ))}
          ...
          <Card className="border-none shadow-2xl bg-white rounded-[3rem] overflow-hidden">
            <CardHeader className="p-8 md:p-12 pb-0 border-b border-gray-50 bg-gray-50/20">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[var(--puembo-green)]">Rendimiento</span>
              </div>
                ...
                <AreaChart data={activityData}>
                  <defs>
                    <linearGradient id="puemboGradient" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%" stopColor="var(--puembo-green)" stopOpacity={0.2}/>
                      <stop offset="95%" stopColor="var(--puembo-green)" stopOpacity={0}/>
                    </linearGradient>
                  </defs>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
                  <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 'bold', fill: '#94a3b8'}} dy={15} />
                  <YAxis axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 'bold', fill: '#94a3b8'}} />
                  <RechartsTooltip contentStyle={{ borderRadius: '1.5rem', border: 'none', boxShadow: '0 25px 50px -12px rgb(0 0 0 / 0.15)' }} />
                  <Area type="monotone" dataKey="cantidad" stroke="var(--puembo-green)" strokeWidth={3} fill="url(#puemboGradient)" activeDot={{ r: 6, strokeWidth: 0 }} />
                </AreaChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {form.form_fields.map((field) => {
              ...
              return (
                <Card key={field.id} className="border-none shadow-2xl bg-white rounded-[3rem] overflow-hidden flex flex-col hover:shadow-emerald-500/5 transition-all">
                  <CardHeader className="p-8 md:p-10 pb-6 border-b border-gray-50 bg-gray-50/30">
                    <div className="space-y-3">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-bold text-gray-400 italic">{respondedCount} respuestas</span>
                      </div>
                    ...
                    {isChartable && stats.length > 0 ? (
                      <div className="flex flex-col space-y-8">
                        <div className="h-[250px] w-full">
                          <ResponsiveContainer width="100%" height="100%">
                            {fieldType === "checkbox" ? (
                              <BarChart data={stats.sort((a, b) => b.value - a.value)} layout="vertical">
                                ...
                                <Bar dataKey="value" radius={[0, 10, 10, 0]} barSize={20} stroke="none">
                                  {stats.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                                </Bar>
                              </BarChart>
                            ) : (
                              <PieChart>
                                <Pie data={stats} cx="50%" cy="50%" innerRadius={60} outerRadius={85} paddingAngle={5} dataKey="value" stroke="none">
                                  {stats.map((_, index) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}
                                </Pie>
                          ...
                          {stats.map((stat, idx) => (
                            <div key={idx} className="flex items-center justify-between text-[11px] py-1">
                              ...
                              <span className="font-black text-gray-900 bg-gray-50 px-2 py-0.5 rounded-md border border-gray-100 whitespace-nowrap">
                                {stat.value} ({((stat.value / respondedCount) * 100).toFixed(1)}%)
                              </span>
                            </div>
                          ))}
                        </div>
                      ...
                      <div className="space-y-4 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                        {filteredSubmissions.length > 0 ? filteredSubmissions.slice(0, 10).map((s, idx) => {
                          const val = s.data[field.label];
                          const empty = isValueEmpty(val);

                          return (
                            <div key={idx} className="p-5 bg-gray-50/50 rounded-2xl border border-gray-100 shadow-sm text-sm group hover:bg-white transition-all overflow-hidden">
                              {empty ? (
                                <span className="text-gray-300 italic font-light">No proporcionado</span>
                              ) : typeof val === 'object' && val._type === 'file' ? (
                                <div className="flex items-center gap-2 text-[var(--puembo-green)] font-bold truncate">
                                  <FileText className="w-4 h-4 shrink-0" /> <span className="truncate">{val.name}</span>
                                </div>
                              ) : (
                                <p className="text-gray-700 font-light leading-relaxed italic truncate">"{String(val)}"</p>
                              )}
                            </div>
                          );
                        }) : (
                          <div className="py-12 text-center flex flex-col items-center gap-3">
                            ...
                            <p className="text-xs text-gray-400 italic uppercase font-bold tracking-widest">Sin respuestas en este periodo</p>
                          </div>
                        )}
                      </div>
                    )}
        ...
        <TabsContent value="individual" className="space-y-8 focus-visible:outline-none">
          {totalSubmissions > 0 ? (
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-6 rounded-[2.5rem] border border-gray-100 shadow-2xl shadow-gray-200/20 sticky top-24 z-20">
                <div className="flex items-center gap-4">
                  ...
                  <div className="min-w-0">
                    <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Expediente</p>
                    <div className="text-sm font-bold truncate">
                      Respuesta <span className="text-[var(--puembo-green)] px-1">{individualIndex + 1}</span> de {totalSubmissions}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-3 w-full sm:w-auto">
                  ...
                  >
                    <ChevronLeft className={cn("w-4 h-4", !isSm && "md:mr-2")} />
                    {!isSm && "Anterior"}
                  </Button>
                  <Button 
                    variant="outline" 
                  ...
                  >
                    {!isSm && "Siguiente"}
                    <ChevronRight className={cn("w-4 h-4", !isSm && "md:ml-2")} />
                  </Button>
                </div>
              </div>
              ...
              <Card className="border-none shadow-2xl bg-white rounded-[3rem] overflow-hidden">
                <div className="p-8 md:p-16 border-b border-gray-50 bg-gray-50/20 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
                  <div className="space-y-2">
                    <div className="flex items-center gap-3">
                      <div className="h-px w-8 bg-[var(--puembo-green)]" />
                      <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[var(--puembo-green)]">Registro Individual</span>
                    </div>
                    <h2 className="text-3xl font-serif font-bold text-gray-900 leading-none">Respuesta #{individualIndex + 1}</h2>
                  </div>
                  <div className="flex items-center gap-4 bg-white px-6 py-4 rounded-3xl border border-gray-100 shadow-sm shadow-gray-200/10 max-w-full overflow-hidden">
                    ...
                    <div className="text-left leading-tight min-w-0">
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Marca Temporal</p>
                      <p className="text-[11px] md:text-xs font-bold text-gray-600 truncate">{formatInEcuador(currentIndividual.created_at, "d MMM, yyyy · HH:mm:ss")}</p>
                    </div>
                  </div>
                </div>
                  ...
                  {form.form_fields.map((field) => {
                    ...
                    return (
                      <div key={field.id} className="group space-y-4">
                        <div className="flex items-center gap-3">
                          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-300 group-hover:text-[var(--puembo-green)] transition-colors">{field.label}</span>
                        </div>
                        <div className="min-h-[2rem]">
                          {empty ? (
                            <span className="text-gray-200 italic font-light text-xl md:text-2xl">No proporcionado</span>
                          ) : isFile ? (
                            <div className="flex items-center gap-5 p-6 bg-emerald-50/50 rounded-[2rem] w-fit border border-emerald-100 shadow-inner group/file transition-all max-w-full overflow-hidden">
                              ...
                              <div className="flex flex-col min-w-0">
                                <span className="text-base font-bold text-emerald-900 leading-tight truncate">{val.name}</span>
                                <span className="text-[10px] font-black uppercase tracking-widest text-[var(--puembo-green)] opacity-60 mt-1">{val.info}</span>
                              </div>
                            </div>
                          ) : Array.isArray(val) ? (
                            <div className="flex flex-wrap gap-3 pt-2">
                              {val.map((v, i) => <span key={i} className="px-5 py-2 bg-gray-50 border border-gray-100 rounded-full text-[10px] font-black uppercase tracking-widest text-gray-600 shadow-sm">{v}</span>)}
                            </div>
                          ) : (
                            <p className="text-xl md:text-2xl text-gray-800 leading-relaxed font-light font-serif italic whitespace-pre-wrap break-words">"{String(val)}"</p>
                          )}
                        </div>
                      </div>
                    );
            ...
            <div className="py-32 text-center bg-white rounded-[3rem] border-2 border-dashed border-gray-100 flex flex-col items-center gap-4 shadow-inner">
              ...
              <p className="text-gray-400 font-bold uppercase text-[10px] tracking-[0.3em]">No hay respuestas en este periodo</p>
            </div>
          )}
        </TabsContent>
      </Tabs>
