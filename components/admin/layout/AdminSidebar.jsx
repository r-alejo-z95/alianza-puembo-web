    ...
    <div className="flex h-screen w-full bg-gray-100 dark:bg-gray-900">
        ...
        <div className="mb-8 flex justify-center border-b border-gray-100 pb-4">
          <Link href="/admin">
            <Image
              ...
              width={150}
              height={97}
              className="object-contain w-37.5 h-auto"
              priority
        ...
        <nav className="flex flex-col space-y-2 grow">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-[hsl(92,45.9%,40%)] transition-colors text-white shadow-2xl"
            >
        ...
        <div className="mt-auto flex flex-col gap-4 border-t border-gray-100 pt-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="flex items-center gap-3 px-3 py-2 rounded-md hover:bg-[hsl(92,45.9%,40%)] transition-colors justify-start text-white w-full"
              >
                <Avatar className="h-8 w-8 shadow-2xl">
                  <AvatarFallback className="text-white bg-(--puembo-black) text-lg font-bold">
      ...
      <div className="lg:hidden fixed top-4 left-4 z-50">
        <Button
          ...
          className="bg-(--puembo-green) text-white rounded-full p-2 shadow-md"
        >
      ...
      {isSidebarOpen && (
        <div
          className="fixed inset-0 bg-(--puembo-black) bg-opacity-50 z-40 lg:hidden"
          onClick={() => setIsSidebarOpen(false)}
        ></div>
      )}
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-(--puembo-green) text-white p-4 flex-col z-50 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out lg:hidden shadow-lg`}
      >
