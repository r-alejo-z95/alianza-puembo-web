...
export default function QuestionImporter({ onImport, onCancel }) {
  ...
  const filteredForms = useMemo(() => {
    let result = [...forms];
    if (searchTerm) {
      result = result.filter(f => f.title.toLowerCase().includes(searchTerm.toLowerCase()));
    }
    if (authorFilter !== "all") {
  ...
  const filteredQuestions = useMemo(() => {
    ...
    return selectedForm.form_fields.filter(f => 
      f.label?.toLowerCase().includes(questionSearchTerm.toLowerCase()) ||
      getFieldLabel(f.type).toLowerCase().includes(questionSearchTerm.toLowerCase())
    );
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-4">
        ...
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-300">Cargando Formularios</p>
      </div>
    );
  }
      ...
      {step === "select-form" ? (
          ...
          <div className="p-6 border-b border-gray-50 space-y-4 bg-gray-50/30">
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1 group">
                ...
                <Input 
                  ...
                  className="pl-12 h-12 rounded-2xl bg-white border-gray-100 text-sm"
                  value={searchTerm}
              ...
              <div className="relative group min-w-[180px]">
                <User className="absolute left-5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 group-focus-within:text-[var(--puembo-green)] transition-colors" />
                <select 
                  value={authorFilter} 
                  onChange={(e) => setAuthorFilter(e.target.value)}
                  className="w-full pl-12 pr-10 h-12 rounded-2xl bg-white border-gray-100 text-sm appearance-none outline-none cursor-pointer"
                >
                  <option value="all">Autores</option>
                  {uniqueAuthors.map(author => (
                    <option key={author.email} value={author.email}>{author.full_name?.split(' ')[0] || author.email}</option>
                  ))}
                </select>
            ...
            <div className="flex gap-2">
              <Button 
                ...
                className={cn("rounded-full text-[10px] uppercase font-black tracking-widest px-4", sortConfig.key === "title" ? "bg-[var(--puembo-green)]/10 text-[var(--puembo-green)]" : "text-gray-400")}
              >
                {sortConfig.key === "title" ? (sortConfig.direction === "asc" ? <SortAsc className="w-3.5 h-3.5 mr-2" /> : <SortDesc className="w-3.5 h-3.5 mr-2" />) : null}
                Nombre
              </Button>
              <Button 
                variant="ghost" 
                ...
                className={cn("rounded-full text-[10px] uppercase font-black tracking-widest px-4", sortConfig.key === "created_at" ? "bg-[var(--puembo-green)]/10 text-[var(--puembo-green)]" : "text-gray-400")}
              >
                {sortConfig.key === "created_at" ? (sortConfig.direction === "asc" ? <SortAsc className="w-3.5 h-3.5 mr-2" /> : <SortDesc className="w-3.5 h-3.5 mr-2" />) : null}
                Fecha
              </Button>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-3">
            {filteredForms.length === 0 ? (
              <div className="py-20 text-center space-y-4 opacity-40">
                ...
                <p className="text-sm font-medium">No se encontraron formularios</p>
              </div>
            ) : (
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <div className="space-y-1.5 min-w-0">
                      <h4 className="font-serif font-bold text-lg text-gray-900 truncate pr-8">{form.title}</h4>
                      <div className="flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-gray-400">
                        <span className="flex items-center gap-1.5"><User className="w-3.5 h-3.5" /> {form.profiles?.full_name?.split(' ')[0] || 'Admin'}</span>
                        <span className="flex items-center gap-1.5"><Calendar className="w-3.5 h-3.5" /> {format(parseISO(form.created_at), "d MMM, yyyy", { locale: es })}</span>
                        <span className="text-[var(--puembo-green)]">{form.form_fields?.length || 0} preguntas</span>
                      </div>
                    </div>
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-8 border-b border-gray-50 bg-gray-50/30">
            <Button variant="ghost" onClick={() => setStep("select-form")} className="mb-4 -ml-4 rounded-full text-gray-400 hover:text-gray-900 text-xs font-bold uppercase tracking-widest">
              <ChevronRight className="w-4 h-4 mr-1 rotate-180" /> Volver a lista
            </Button>
            <h3 className="text-2xl font-serif font-bold text-gray-900">{selectedForm.title}</h3>
            <p className="text-xs text-gray-400 mt-1 uppercase font-black tracking-widest">Selecciona las preguntas a importar</p>
          </div>
          ...
          <div className="px-6 py-4 border-b border-gray-50 bg-white sticky top-0 z-20">
            <div className="relative group">
              ...
              />
              {questionSearchTerm && (
                <button onClick={() => setQuestionSearchTerm("")} className="absolute right-4 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-100 rounded-full">
          ...
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between bg-white">
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Seleccionar filtradas</span>
            </div>
            <span className="text-[10px] font-black uppercase tracking-widest text-[var(--puembo-green)]">{selectedQuestionIds.length} seleccionadas</span>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-3">
            {filteredQuestions.length === 0 ? (
              <div className="py-20 text-center opacity-40">
                <p className="text-sm font-medium font-serif italic">No hay preguntas que coincidan con la búsqueda</p>
              </div>
            ) : (
              filteredQuestions.map((field, index) => (
                ...
                >
                  ...
                  <div className="space-y-1 min-w-0">
                    ...
                    <p className="font-bold text-gray-900 leading-snug">{field.label || "Sin etiqueta"}</p>
                    {field.help_text && <p className="text-[10px] text-gray-400 italic line-clamp-1">{field.help_text}</p>}
                  </div>
                </label>
              ))
            )}
          ...
          <div className="p-8 bg-white border-t border-gray-50">
            ...
            >
              <Plus className="w-5 h-5 mr-2" /> Importar {selectedQuestionIds.length} {selectedQuestionIds.length === 1 ? 'pregunta' : 'preguntas'}
            </Button>
          </div>
        </div>
      )}
